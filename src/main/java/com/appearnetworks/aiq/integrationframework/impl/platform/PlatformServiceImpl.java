...
public class PlatformServiceImpl implements PlatformService {
    ...
    public void fetchOrgRootMenu() {
        try {
            ...
            throw new PlatformUnavailableException(e.getMessage());
        } catch (HttpMessageConversionException | RestClientException e) {
            throw new PlatformException(e.getMessage());
        }
    }

    public String fetchPlatformUserToken() {
    ...
    private void invalidatePlatformUserToken() {
                ...
                case UNAUTHORIZED:
                    ...
                    if (fetchPlatformUserToken() != null)
                        try {
                            ...
                            throw new PlatformUnavailableException(e2.getMessage());
                        } catch (HttpMessageConversionException | RestClientException e2) {
                            throw new PlatformException(e2.getMessage());
                        }
                    else
                        throw new UnauthorizedException();
    ...
    @Override
    public PlatformUser validatePlatformUserToken(String token) {
        ObjectNode request = mapper.valueToTree(new PlatformUserToken(token));
    ...
    @Override
    public List<PlatformUser> fetchUsers() {
        try {
            return Arrays.asList(getForObject(fetchIntegrationLink(USERS), PlatformUser[].class));
        } catch (UnauthorizedException e) {
            if (fetchPlatformUserToken() != null)
                return Arrays.asList(getForObject(fetchIntegrationLink(USERS), PlatformUser[].class));
            else
    ...
    @Override
    public PlatformUser fetchUser(String id) {
        try {
            return getForObjectOrNull(fetchIntegrationLink(USERS) + "/" + id, PlatformUser.class);
        } catch (UnauthorizedException e) {
            if (fetchPlatformUserToken() != null)
                return getForObjectOrNull(fetchIntegrationLink(USERS) + "/" + id, PlatformUser.class);
            else
    ...
    public RestTemplate getRestTemplateWithAuth() {
        ClientHttpRequestInterceptor interceptor = new HeaderHttpRequestInterceptor(fetchPlatformUserToken());
    ...
    private RuntimeException reportHttpError(HttpStatusCodeException e) {
        if (e.getStatusCode() == HttpStatus.SERVICE_UNAVAILABLE)
            ...
            return new PlatformException(e.getStatusCode(), e.getResponseBodyAsString());
    }
